 ...
 ***********************************************************/
void ParserBase::Run(const char *pszFastaFileName, const char *pszDBFileName, const char *pszDBLabel,
								 ...
								 const char *pszDBRelease)
{
	...
	if(fscanf(m_fastaFile, "%c", &c) != 1)
		ThrowError("ParserBase::Run", "file is empty");
	if(c != '>')
		ThrowError("ParserBase::Run", "Entry should start with '>'");
 ...
 ***********************************************************/
void ParserBase::ParserCallBack(void)
{
	printf("- parsing...\n");
	while(Read())
		Write();
...
bool ParserBase::Read(void)
{
	...
	if(!m_pszAC)
		ThrowError("ParserBase::Read", "No AC defined");
    
	m_pszID = strtok(NULL, " ");	
	if(!m_pszID)
		ThrowError("ParserBase::Read", "In AC='%s' No ID defined", m_pszAC);

	m_pszDE = strtok(NULL, "|");
	if(!m_pszDE)
		ThrowError("ParserBase::Read", "In AC='%s' No DE defined", m_pszAC);
 ...
 ***********************************************************/
char *ParserBase::RemoveEndWhiteCharacters(char *psz)
{
 ...
 ***********************************************************/
void ParserBase::ReadPtm(char *psz)
{
	...
	for(int i=0; i<aToken.GetNbElt(); i++){
		...
		if(!p || sscanf(p, "%d", &iPos) != 1)
			ThrowError("ParserBase::ReadPtm", "Ptm are bad formated");
 ...
 ***********************************************************/
void ParserBase::AddUnknownModRes(const char *psz)
{
