...
bool data::loadPeaksPOP(File &fp) 
{
	...
	while (fgets(line, 255, fp)) {
		...
		// RECUPERE TITLE, PEPMASS ET CHARGE, puis les pics jusqu'� END ION
		if (isdigit(line[0])) {
			GOT_PEAKS = true;
			int j = 0;
			while (!((line[j] == ' ') | (line[j] == '\t'))) {
...
bool data::loadPeaksDTA(File &fp) 
{
	...
	while (fgets(line, 255, fp)) {
		...
		// ici, line doit contenir la premi�re ligne d'un spectre.
		strcpy(tag, line);
		...
		// lecture des pics jusqu'au premier blanc ou jusqu'� la fin
		while (fgets(line, 255, fp)) {
			// remove special characters
			int lenght = strlen(line);
			if ((line[strlen(line)-1] == '\n') | (line[strlen(line)-1] == '\r')) {
...
void  data::markParentMass()
{
	for (int i = 0; i < peakNb; i++) {
		if (fabs(peakList[i].mass - parentMassRAW) < runManParam->FRAGMENT_ERROR2) {
			...
			// vire tous les pics isopopiques de la masse trouv�e (afin que leur intensit�
...
void data::normInt()
{
	// les pics sont tri�s par intensit�, mais il se peut que le premier (et quelques
	// suivants) soient la masse parente ou des isotopes de la masse parente.
	// Donc je prend comme intensit� la plus �lev�e le premier pic qui n'a pas �t�
	// marqu� comme masse parente ou isotope de la masse parente
	double max = 0.0;
	int i = 0;
