...
runManager::~runManager() 
{
  if (runManParam	  != NULL) {delete runManParam; runManParam = NULL; memCheck.runManParam--;}
  if (aaParam       != NULL) {delete aaParam;     aaParam     = NULL; memCheck.aa--;}
  if (ionParamTOFTOF1 != NULL) {delete ionParamTOFTOF1;	ionParamTOFTOF1	= NULL;					  memCheck.ion--;}
  if (ionParamQTOF1	  != NULL) {delete ionParamQTOF1;	ionParamQTOF1	= NULL;					  memCheck.ion--;}
  if (ionParamQTOF2	  != NULL) {delete ionParamQTOF2;	ionParamQTOF2	= NULL;					  memCheck.ion--;}
  if (ionParamQTOF3	  != NULL) {delete ionParamQTOF3;	ionParamQTOF3	= NULL;					  memCheck.ion--;}
  if (spectrumData	  != NULL) {delete spectrumData;	spectrumData	= NULL;					  memCheck.data--;}
  if (popiResults	  != NULL) {delete popiResults;		popiResults		= NULL;					  memCheck.results--;}
  if (allRunStats	  != NULL) {delete allRunStats;		allRunStats		= NULL;					  memCheck.allrunstatistics--;}
  if (gp_parameters	  != NULL) {delete gp_parameters;	gp_parameters	= NULL;					  memCheck.functionScore--;}
  if (scoreFunction[0]!= NULL) {delete scoreFunction[0];scoreFunction[0]= NULL;					  memCheck.functionScore--;}
  if (scoreFunction[1]!= NULL) {delete scoreFunction[1];scoreFunction[1]= NULL;					  memCheck.functionScore--;}
  if (scoreFunction[2]!= NULL) {delete scoreFunction[2];scoreFunction[2]= NULL;					  memCheck.functionScore--;}
  if (popitam		  != NULL) {delete popitam;			popitam			= NULL;					  memCheck.compare--;}
}
...
void runManager::init(int argc,	char** argv) 
{
  ...
  if (runManParam->r_CHECK) prepareOutDir();
  initAaParam();
  initIonParam();
  initPopiResult();
  initAllRunStats();
  initScoreFunctions();						   // MISE EN MEMOIRE DES FONCTIONS	DE SCORE //
}
...
void runManager::initIonParam()
{
  ...
  ionParamTOFTOF1->init(runManParam->FILE_ERROR_NAME, runManParam->PROBS_TOFTOF1_INFILE, aaParam); memCheck.ion++;
  ionParamQTOF1   = new ion();
  ionParamQTOF1->init(runManParam->FILE_ERROR_NAME, runManParam->PROBS_QTOF1_INFILE, aaParam); memCheck.ion++;  
  ionParamQTOF2   = new ion();
  ionParamQTOF2->init(runManParam->FILE_ERROR_NAME, runManParam->PROBS_QTOF2_INFILE, aaParam); memCheck.ion++;
  ionParamQTOF3   = new ion();
  ionParamQTOF3->init(runManParam->FILE_ERROR_NAME, runManParam->PROBS_QTOF3_INFILE, aaParam); memCheck.ion++;
...
{
  popiResults =	new	result;																		  memCheck.results++;
...
{
  allRunStats =	new	allrunstatistics;															  memCheck.allrunstatistics++;
  allRunStats->init();
}
...
{
  gp_parameters	 = new PARAMETERS;																  memCheck.functionScore++;
  gp_parameters->init(runManParam->GPPARAMETERS_INFILE);

  scoreFunction[0] = new TREE;																	  memCheck.functionScore++;
  ...
  scoreFunction[1] = new TREE;																	  memCheck.functionScore++;
  ...
  scoreFunction[2] = new TREE;																	  memCheck.functionScore++;
...
{   
  if (runManParam->s_IDSET)	writePerformanceIDSET(runManParam->FILEOUT);
 //	 if	(runManParam->s_MIXSET)	 writePerformanceMIXSET(runManParam->FILEOUT);
  ...
  if (runManParam->r_NORMAL)  {
	if (popiResults->protList->firstElement->following->FILLED)	{								  // S'IL Y	A AU MOINS UN ELEMENT
...
void runManager::writePerformanceIDSET(File& fp) {
  ...
  for (int i = 0; i	< runManParam->RESULT_NB; i++) {
	...
	if ((runManParam->SPECTRUM_NB -	allRunStats->notInDtb) == 0) fprintf(fp, "(%s)\n", "0%");
	else	fprintf(fp,	"(%.1f%s)\n",  (float)popiResults->tabRank[i]/(float)(runManParam->SPECTRUM_NB - allRunStats->notInDtb)*100, "%");
  }
  ...
  if (allRunStats->truePos == 0) fprintf(fp, "\n\nPercent of True Positive cases	  =	%s", "0%");
  else	 fprintf(fp, "\n\nPercent of True Positive cases	  =	%5.1f%s", float(allRunStats->truePos) /(float)(runManParam->SPECTRUM_NB	- allRunStats->notInDtb) * 100,"%");
  if (allRunStats->falsePos	== 0) fprintf(fp, "\n\nPercent of False	Positive cases	   = %s", "0%");
  else	 fprintf(fp, "\nPercent	of False Positive cases	   = %5.1f%s",	 float(allRunStats->falsePos)/(float)(runManParam->SPECTRUM_NB - allRunStats->notInDtb)	* 100, "%");
  fprintf(fp, "\n\n%i sequences	were not in	the	database\n", allRunStats->notInDtb);
}
