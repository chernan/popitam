...
void Taxonomy::Load(const char *pszFileName, TE_Taxonomy eTaxonomy)
{
	...
	if(pszFileName){
		...
		switch(eTaxonomy){
			case NCBI_TAXO:
				LoadNCBI(pszFileName, aTaxon);	//use the taxonomy file
				break;

			
			case NEWT_TAXO:
				LoadNewt(pszFileName, aTaxon);	//use the taxonomy file
				break;

			default:
				ThrowError("Taxonomy::Load", "Unknown taxonomy %d", eTaxonomy);
...
void Taxonomy::LoadNewt(const char *pszFileName, DynamicArray<Taxon *> &aTaxon)
{
	printf("- Loading Newt taxonomy...\n");
	File	file;
	file.Open(pszFileName, "r");

	char	sz[MAX_LINE_LEN];
	int		iId, iParentId;
	char	*psz;

	
	while(fgets(sz, MAX_LINE_LEN, file)){	//"taxon name" iId iParentId\n
		
		psz = sz;
		while(*(++psz) != '"');
		*psz = '\0';

		if(sscanf(psz+1, "%d %d", &iId, &iParentId) != 2)
			ThrowError("Taxonomy::LoadAllTaxon", "file bad format");

		//poubellise les taxid negatif s'ils en existe encore ...:-(((
		if( (iId<0) || (iParentId<0) )
			continue;
