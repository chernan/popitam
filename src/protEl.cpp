...
void protEl::fillIt(element* cE, int i, char* title, char* seqAsInDtb, float PMraw, float PM, int specID, char* fileErrorName) 
{
  FILLED           = TRUE;
	...
	protIds[0].dtbId = cE->Peptide->myProt[i]->m_reloadDBEntry.m_iDBFileIndex; 
  protIds[0].offId = cE->Peptide->myProt[i]->m_reloadDBEntry.m_uiEntryOffset;
  
  strcpy(protIds[0].AC, cE->Peptide->myProt[i]->AC);
  strcpy(protIds[0].ID, cE->Peptide->myProt[i]->ID);
  strcpy(protIds[0].DE, cE->Peptide->myProt[i]->DE);
  similarProtNb ++;
